...
## subset this data frame to get only the mean and standard deviation columns

tfeatures <- tfeatures[,grep("mean\\(|std\\(",col_names)]
...
xfeatures <- xfeatures[,grep("mean\\(|std\\(",col_names)]
...
## no longer necessary we did it before
